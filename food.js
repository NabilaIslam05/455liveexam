...
const showMealInfoDiv = (data, mealInput, mealNumber) => {
    const meal = data.meals;

    if(meal){
        meal.slice(-mealNumber).forEach(element => {
        createMealInfoDiv(element, mealInput);
        });
    }
    else{
        const noMealFound = document.getElementById('no-meal-found');
        noMealFound.innerText = `No Search Result for ${mealInput}!`;
    }
}
const searchMeal = () =>{
    const mealInput = document.getElementById('meal-input').value;
    const mealNumber = document.getElementById('meal-number').value;
    ...
    if(mealInput){
        ...
        .then(res => res.json())
        .then(data => {
            showMealInfoDiv(data, mealInput, mealNumber);
        }
        )
    }
    else{
        const noMealFound = document.getElementById('no-meal-found');
        noMealFound.innerText = `Empty Search!`;
    }
}
